...
function dbPath() {
  ...
  if (platform() === "darwin")
    return join(home, "Library", "Application Support", IDENT, "app.db");
  if (platform() === "win32")
    return join(
...
if (!existsSync(DB_FILE)) {
  console.error(
    `Database not found at ${DB_FILE}. Launch the app once so the migrations create it, then re-run.`,
  );
  process.exit(1);
}
...
const ymdhms = (d) =>
  `${ymd(d)} ${p2(d.getHours())}:${p2(d.getMinutes())}:${p2(d.getSeconds())}`;
...
const phone = () => {
  const p = pick(["5", "6", "7"]);
  let r = "";
  for (let i = 0; i < 8; i++) r += randInt(0, 9);
  return `+213 0${p}${r.slice(0, 1)} ${r.slice(1, 3)} ${r.slice(3, 5)} ${r.slice(5, 7)}`;
};
const nin = () => {
  let s = "";
...
if (patientCount > 0 && MODE !== "reset") {
  console.error(
    "Database already has patients. Re-run with --reset to wipe and reseed, or --clear to wipe only.",
  );
  process.exit(1);
}
...
const attr = new Map(
  all("SELECT id, key, field_type FROM attribute_definitions").map((r) => [
    r.key,
    { id: Number(r.id), field_type: r.field_type },
  ]),
);
function setProductAttrs(productId, dict) {
  for (const [key, value] of Object.entries(dict)) {
    const m = attr.get(key);
    ...
    run(
      "INSERT INTO product_attribute_values (product_id, attribute_id, value_text, value_num, value_options) VALUES (?,?,?,?,?)",
      productId,
      m.id,
      text,
      num,
...
function setPatientTags(patientId, tags) {
  const m = attr.get("client_tags");
  if (!m || !tags.length) return;
  run(
    "INSERT INTO patient_attribute_values (patient_id, attribute_id, value_text, value_num, value_options) VALUES (?,?,?,?,?)",
    patientId,
    m.id,
    null,
    null,
...
const setI = (key, def) => {
  const r = get1("SELECT value FROM settings WHERE key=?", key);
...
const TVA = setI("tva_rate", 1900),
  TIMBRE_RATE = setI("timbre_rate", 100),
  TIMBRE_MIN = setI("timbre_min", 0),
  TIMBRE_MAX = setI("timbre_max", 0);
const INV_PREFIX = setS("invoice_prefix", ""),
  INV_PAD = Math.max(1, setI("invoice_padding", 6));
let invoiceNext = setI("invoice_next", 1);

// ── payers ──────────────────────────────────────────────────────────────────
const mkPayer = (name, type, cov, notes) =>
  lastId(
    "INSERT INTO payers (name,type,default_coverage_pct,notes) VALUES (?,?,?,?)",
    name,
    type,
    cov,
    notes,
...
const payers = [
  ...
  },
  {
    id: mkPayer(
      ...
      "Mutuelle",
      5000,
      "Assurance complémentaire entreprise.",
    ),
    cov: 5000,
    prefix: "SAA",
  },
...
for (const s of supplierDefs) {
  const id = lastId(
    ...
    phone(),
    s.email,
    `${pick(STREETS)}, ${pick(CITIES)}`,
    s.note,
  );
  supplierIds[s.name] = id;
  const buy1 = da(randInt(80, 240) * 1000),
    buy2 = da(randInt(40, 160) * 1000);
  const led = (type, amt, note, ref, when) =>
    run(
      "INSERT INTO supplier_ledger (supplier_id,type,amount,note,ref,created_at) VALUES (?,?,?,?,?,?)",
      id,
      type,
      amt,
      note,
  ...
  led(
    "purchase",
    buy1,
    "Commande montures/verres",
    `BL-${randInt(1000, 9999)}`,
  ...
  led(
    ...
    "Réassort",
    `BL-${randInt(1000, 9999)}`,
    daysAgo(randInt(30, 110)),
  );
  if (chance(0.8))
    led(
      "payment",
      -Math.round((buy1 + buy2) * (0.4 + rand() * 0.4)),
      "Versement",
      `VIR-${randInt(1000, 9999)}`,
      daysAgo(randInt(10, 60)),
    );
}
...
for (const c of [
  ...
  categoryIds[c] = lastId("INSERT INTO categories (name) VALUES (?)", c);
const brandIds = {};
for (const b of [
  "Ray-Ban",
  "Oakley",
  ...
  brandIds[b] = lastId("INSERT INTO brands (name) VALUES (?)", b);
...
function addProduct(a) {
  ...
  const productId = lastId(
    ...
    a.expiry ? ymd(a.expiry) : null,
    da(a.purchase),
    da(a.selling),
    hasVariants ? 0 : a.quantity,
    hasVariants ? 0 : a.min_stock,
    a.supplier ?? null,
    a.categoryName ? categoryIds[a.categoryName] : null,
    a.brand ? brandIds[a.brand] : null,
    a.supplier ? supplierIds[a.supplier] : null,
    ymdhms(daysAgo(randInt(120, 260))),
    ymdhms(daysAgo(randInt(120, 260))),
  );
  if (a.attrs) setProductAttrs(productId, a.attrs);
  if (!hasVariants && !isService && a.quantity > 0)
    run(
      "INSERT INTO stock_movements (product_id,type,quantity_change,note,created_at) VALUES (?,?,?,?,?)",
      productId,
      "delivery",
      a.quantity,
      "Initial stock",
  ...
  if (hasVariants) {
    for (const v of a.variants) {
      ...
      sellables.push({
        ...
        description: `${a.brand ? a.brand + " " : ""}${a.name} — ${v.color}${v.size ? ` ${v.size}` : ""}`,
        unit_price: price,
        category: a.category,
        avail: v.qty,
      });
    ...
    sellables.push({
      ...
      product_id: productId,
      variant_id: null,
      description: `${a.brand ? a.brand + " " : ""}${a.name}`,
      unit_price: da(a.selling),
      category: a.category,
      avail: a.quantity,
    });
  } else {
...
const logAct = (pid, type, desc, refId, when) =>
  run(
    "INSERT INTO patient_activity (patient_id,type,description,ref_id,created_at) VALUES (?,?,?,?,?)",
    pid,
    type,
    desc,
    refId ?? null,
...
for (let i = 0; i < 30; i++) {
  ...
  const id = lastId(
    ...
    name,
    phone(),
    chance(0.2) ? phone() : null,
    chance(0.4) ? `${fname.toLowerCase()}.${randInt(1, 999)}@gmail.com` : null,
    `${randInt(1, 80)} ${pick(STREETS)}, ${pick(CITIES)}`,
    dob,
    chance(0.7) ? nin() : null,
    payer ? payer.id : null,
    payer ? payer.cov : 0,
    payer ? policyNo(payer.prefix) : null,
    chance(0.25)
      ? pick([
          "Client fidèle.",
  ...
  if (chance(0.75)) {
    ...
    for (let r = 0; r < nRx; r++) {
      ...
      const rxId = lastId(
        ...
        ymd(exam),
        diopter(-6, 3),
        hasCyl ? diopter(-2.5, -0.25) : null,
        hasCyl ? randInt(0, 180) : null,
        add,
        randInt(29, 34),
        diopter(-6, 3),
        hasCyl ? diopter(-2.5, -0.25) : null,
        ...
        randInt(29, 34),
        lensType,
        lensType === "progressive" ? randInt(14, 20) : null,
        lensType === "progressive" ? randInt(14, 20) : null,
        pick(PRESCRIBERS),
        ymd(expiry),
        chance(0.2) ? "Renouvellement, pas de changement majeur." : null,
        ymdhms(exam),
      );
      rxIds.push(rxId);
...
for (const pt of sample(patients, 22)) {
  if (chance(0.7)) {
    ...
    let status = "done",
      rxLink = null;
    if (pt.rxIds.length && chance(0.6)) {
      rxLink = pt.rxIds[0];
      status = "done";
    } else status = pick(["done", "done", "no_show", "cancelled"]);
    const apptId = lastId(
      "INSERT INTO appointments (patient_id,starts_at,duration_min,optometrist,reason,status,prescription_id,created_at) VALUES (?,?,?,?,?,?,?,?)",
      pt.id,
      at(d, randInt(9, 16), pick([0, 30])),
      30,
      pick(OPTOMETRISTS),
      "Examen de vue",
      status,
  ...
  if (chance(0.4)) {
    ...
    run(
      "INSERT INTO appointments (patient_id,starts_at,duration_min,optometrist,reason,status) VALUES (?,?,?,?,?, 'booked')",
      pt.id,
      at(d, randInt(9, 16), pick([0, 30])),
      30,
      pick(OPTOMETRISTS),
...
function createSale(o) {
  ...
  const subtotal = items.reduce(
    (s, it) => s + Math.max(0, it.unit_price * it.quantity - it.item_discount),
    0,
  );
  const discount =
    o.discountType === "percent"
      ? Math.floor((subtotal * o.discountValue) / 10000)
      : o.discountValue || 0;
  const total = Math.max(subtotal - discount, 0);
  const tax_amount =
    TVA > 0 && total > 0
      ? total - Math.floor((total * 10000) / (10000 + TVA))
      : 0;
  let timbre = 0;
  ...
  const covered = o.payerId
    ? Math.min(Math.max(Math.floor((total * o.coverage) / 10000), 0), total)
    : 0;
  ...
  if (o.payerId)
    claimId = lastId(
      "INSERT INTO claims (sale_id,payer_id,covered_amount,status,created_at) VALUES (?,?,?, 'pending', ?)",
      saleId,
      o.payerId,
      covered,
      when,
  ...
  if (hasLens)
    jobId = lastId(
      "INSERT INTO jobs (sale_id,patient_id,prescription_id,status,created_at,updated_at) VALUES (?,?,?, 'ordered', ?, ?)",
      saleId,
      o.patientId,
      o.prescriptionId ?? null,
      when,
  ...
  for (const it of items) {
    const lineTotal = Math.max(
      it.unit_price * it.quantity - it.item_discount,
      0,
    );
    run(
      "INSERT INTO sale_items (sale_id,product_id,variant_id,description,unit_price,quantity,item_discount,line_total) VALUES (?,?,?,?,?,?,?,?)",
      saleId,
      it.product_id ?? null,
      it.variant_id ?? null,
      it.description,
    ...
    if (it.variant_id) {
      run(
        "UPDATE product_variants SET quantity = quantity - ?, updated_at = ? WHERE id = ?",
        it.quantity,
        when,
        it.variant_id,
      );
      run(
        "INSERT INTO stock_movements (product_id,variant_id,type,quantity_change,note,created_at) VALUES (?,?, 'sale', ?, ?, ?)",
        it.product_id ?? null,
        it.variant_id,
        -it.quantity,
        `Sale #${saleId}`,
      ...
      run(
        "INSERT INTO stock_movements (product_id,type,quantity_change,note,created_at) VALUES (?, 'sale', ?, ?, ?)",
        it.product_id,
        -it.quantity,
        `Sale #${saleId}`,
        when,
  ...
  if (cashPaid > 0)
    run(
      "INSERT INTO payments (sale_id,amount,method,note,paid_at) VALUES (?,?,?, 'Initial payment', ?)",
      saleId,
      cashPaid,
      o.method,
      when,
...
while (salesMade < 46 && attempts < 200) {
  ...
  if (roll < 0.55) {
    ...
    prescriptionId = pt.rxIds[0] ?? null;
  ...
  let initialPayment;
  if (stateRoll < 0.55)
    initialPayment = 1e12; // paid in full
  else if (stateRoll < 0.85)
    initialPayment = Math.round(goods * (0.3 + rand() * 0.4)); // partial
  ...
  const saleId = createSale({
    ...
    discountValue,
    initialPayment,
    payerId: useInsurer ? pt.payerId : null,
    coverage: useInsurer ? pt.coverage : 0,
    note: chance(0.15)
      ? pick([
  ...
  if (stateRoll >= 0.55 && stateRoll < 0.85 && chance(0.5)) {
    ...
    if (bal > 0) {
      ...
      run(
        "INSERT INTO payments (sale_id,amount,method,note,paid_at) VALUES (?,?,?, 'Acompte', ?)",
        saleId,
        pay,
        pick(PAY_METHODS),
        ymdhms(payDate < NOW ? payDate : NOW),
      );
      run(
        "UPDATE sales SET amount_paid = amount_paid + ?, balance = balance - ?, status = CASE WHEN balance - ? <= 0 THEN 'paid' ELSE 'partial' END WHERE id = ?",
        pay,
        pay,
        pay,
        saleId,
...
for (const c of createdSales) {
  ...
  run(
    ...
    pick(LABS),
    ymd(expected),
    chance(0.2) ? "Verres en rupture côté labo." : null,
    status,
    delivered,
    ymdhms(updatedAt),
    c.jobId,
...
for (const c of createdSales) {
  ...
  const submittedAt = new Date(
    Math.min(NOW.getTime(), c.date.getTime() + randInt(2, 10) * DAY),
  );
  const paidAt = new Date(
    Math.min(NOW.getTime(), submittedAt.getTime() + randInt(10, 45) * DAY),
  );
  const roll = rand();
  if (roll < 0.2) {
    /* pending */
    ...
    run(
      "UPDATE claims SET status='submitted', claim_ref=?, submitted_at=? WHERE id=?",
      ref,
      ymdhms(submittedAt),
      c.claimId,
    );
  else if (roll < 0.65) {
    const part = Math.round(c.covered * (0.4 + rand() * 0.3));
    run(
      "UPDATE claims SET status='partial', claim_ref=?, submitted_at=?, paid_amount=? WHERE id=?",
      ref,
      ymdhms(submittedAt),
      part,
      c.claimId,
    ...
    run(
      "UPDATE claims SET status='paid', claim_ref=?, submitted_at=?, paid_amount=?, paid_at=? WHERE id=?",
      ref,
      ymdhms(submittedAt),
      c.covered,
      ymdhms(paidAt),
    ...
    run(
      "UPDATE claims SET status='rejected', claim_ref=?, submitted_at=? WHERE id=?",
      ref,
      ymdhms(submittedAt),
      c.claimId,
    );
...
for (const c of ordered) {
  ...
  const item = get1(
    `SELECT si.id, si.product_id, si.quantity, si.line_total, p.item_type
     ...
     WHERE si.sale_id = ? AND si.product_id IS NOT NULL AND p.item_type <> 'service' AND si.quantity > 0
       AND si.product_id NOT IN (${[...lensIds].join(",") || "0"}) LIMIT 1`,
    c.saleId,
  );
  if (!item) continue;
  ...
  const cnId = lastId(
    "INSERT INTO credit_notes (sale_id,patient_id,total,method,notes,created_at) VALUES (?,?,?,?,?,?)",
    c.saleId,
    c.patientId,
    perUnit,
    method,
  ...
  run(
    "INSERT INTO credit_note_items (credit_note_id,sale_item_id,product_id,description,quantity,line_total) VALUES (?,?,?,?,?,?)",
    cnId,
    item.id,
    item.product_id,
    "Retour article",
  ...
  run(
    "INSERT INTO stock_movements (product_id,type,quantity_change,note,created_at) VALUES (?, 'adjustment', 1, ?, ?)",
    item.product_id,
...
for (const t of [
  ...
  console.log(`  ${t.padEnd(18)} ${c(t)}`);
console.log("── integrity checks ──");
console.log(
  "  sales by status:",
  JSON.stringify(
    all("SELECT status, COUNT(*) AS n FROM sales GROUP BY status"),
  ),
);
console.log(
  "  claims by status:",
  JSON.stringify(
    all("SELECT status, COUNT(*) AS n FROM claims GROUP BY status"),
  ),
);
console.log(
  "  jobs by status:",
  JSON.stringify(all("SELECT status, COUNT(*) AS n FROM jobs GROUP BY status")),
);
console.log(
  "  negative product stock:",
  sum("SELECT COUNT(*) AS v FROM products WHERE quantity < 0"),
);
console.log(
  "  negative variant stock:",
  sum("SELECT COUNT(*) AS v FROM product_variants WHERE quantity < 0"),
);
console.log(
...
console.log(
  ...
  (
    sum("SELECT COALESCE(SUM(balance),0) AS v FROM sales") / 100
  ).toLocaleString(),
);
db.close();
