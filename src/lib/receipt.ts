...
export function buildReceiptLines(args: {
  ...
  lines.push(
    line(settings.shop_name || t("receipt.shopFallback"), "center", true, true),
  );
  if (cfg.header_text) lines.push(line(cfg.header_text, "center"));
  if (cfg.show_address && settings.shop_address)
    lines.push(line(settings.shop_address, "center"));
  if (cfg.show_phone && settings.shop_phone)
    lines.push(
      line(t("receipt.tel", { phone: settings.shop_phone }), "center"),
    );
  lines.push(line(rule));
  lines.push(
    line(
      t("receipt.invoice", { number: sale.invoice_number ?? `#${sale.id}` }),
    ),
  );
  ...
  for (const it of items) {
    ...
    const left =
      `${cfg.show_qty ? `${it.quantity} x ` : ""}${cfg.show_unit_price ? formatDZD(it.unit_price, sym) : ""}`.trim();
    lines.push(line(twoCol(`  ${left}`, formatDZD(it.line_total, sym), width)));
  }

  lines.push(line(rule));
  lines.push(
    line(twoCol(t("receipt.totalTtc"), formatDZD(sale.total, sym), width)),
  );
  if (cfg.show_tax && sale.tax_amount > 0)
    lines.push(
      line(
        twoCol(
          `  ${t("receipt.inclTva")}`,
          formatDZD(sale.tax_amount, sym),
          width,
        ),
      ),
    );
  if (cfg.show_timbre && sale.timbre_amount > 0)
    lines.push(
      line(
        twoCol(
          t("receipt.droitDeTimbre"),
          formatDZD(sale.timbre_amount, sym),
          width,
        ),
      ),
    );
  if (claim && claim.covered_amount > 0)
    lines.push(
      line(
        twoCol(
          t("receipt.covers", { payer: claim.payer_name }),
          `-${formatDZD(claim.covered_amount, sym)}`,
          width,
        ),
      ),
    );
  ...
  const patientTotal =
    sale.total + sale.timbre_amount - (claim?.covered_amount ?? 0);
  lines.push(
    line(
      twoCol(
        claim ? t("receipt.patientTotal") : t("receipt.total"),
        formatDZD(patientTotal, sym),
        width,
      ),
      "left",
      true,
  ...
  lines.push(
    line(twoCol(t("receipt.paid"), formatDZD(sale.amount_paid, sym), width)),
  );
  lines.push(
    line(twoCol(t("receipt.balance"), formatDZD(sale.balance, sym), width)),
  );
  lines.push(line(rule));
