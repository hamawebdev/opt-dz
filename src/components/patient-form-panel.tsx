...
/**
 * The full patient form, host-agnostic: the patient-form page wraps it in a
 * Card and navigates on save; the POS quick-create dialog auto-selects the
 * new client into the cart instead. Same fields, validation, duplicate guard
 * and simple-mode collapse everywhere.
 */
export function PatientFormPanel({
  ...
  patientId?: number;
  onSaved: (id: number, fullName: string) => void;
  onCancel: () => void;
}) {
  ...
  async function persist(values: FormValues) {
    try {
      ...
      onSaved(savedId, values.full_name.trim());
    } catch (err) {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {isEdit ? t("patients.saveChanges") : t("patients.createPatient")}
            </Button>
          </div>
        </form>
      </Form>
